...
**为什么这里把 V 定义为加权平均值是合适的**：当前实现里 `V` 取就绪队列 `vruntime` 的**按权重平均**，能与 `nice -> weight` 的公平目标保持一致；而平均值作为“中心水位”也有稳定解释（至少会有任务不高于该水位），使 eligible 判定在本实现下直观且可验证。
...
更多与实验、演示、单测相关的文档（同目录 `docs/report/`）：
...
- [`linux_中_nice_命令详解.md`](./linux_中_nice_命令详解.md) — Linux 下 nice 含义与用法（背景）

完整基准与自动化脚本说明见上级目录：[`../eevdf-nice-benchmark.md`](../eevdf-nice-benchmark.md)。
